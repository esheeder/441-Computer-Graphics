...
void generateEnvironmentDL() {
    ...
    // DRAW OUR GRID
	float diff[4] = { 0.07568f, 0.61424f, 0.07568f, 1.0f };
...
void normalKeysDown( unsigned char key, int x, int y )  {
	...
	if (key == '1') {
		if (light1) glDisable(GL_LIGHT0);
		else glEnable(GL_LIGHT0);
		light1 = !light1;
	}
	if (key == '2') {
		if (light2) glDisable(GL_LIGHT1);
...
void initializeOpenGL( int argc, char *argv[] ) {
    ...
    float diffuseCol[4] = { 0.0, 0.2, 0.1, 1.0 };           // TODO #06: Change the colors!
