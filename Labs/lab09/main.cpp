...
void initScene() {
	...
	for (int i = 0; i < numSprites; i++) {
		...
		int x = numSprites / 2 - (rand() % numSprites);
		int z = i - numSprites / 2;
		treeSpritePositions.push_back(Point(x, 0, z));
	}
...
void renderScene(void) {
	...
	Vector view = Vector(-cameraXYZ.getX(), 0.0f, -cameraXYZ.getZ());
	view.normalize();
	sortedTrees.clear();
	for (int i = 0; i < treeSpritePositions.size(); i++) {
		Point treeLocation = treeSpritePositions.at(i);
		Vector CP = Vector(treeLocation.at(0) - cameraXYZ.at(0), 0.0, treeLocation.at(2) - cameraXYZ.at(2));
		double theta = acos(dot(view, CP)/(view.mag() * CP.mag()));
		double distance = CP.mag() * cos(theta);
		sortedTrees.insert(pair<double, int>(-distance, i));
	}

	// TODO #3: Compute rotation of tree to face camera
	Vector camera = Vector(cameraXYZ.getX(), 0.0f, cameraXYZ.getZ());
	camera.normalize();
	Vector tree = Vector(0, 0, 1);
	

	float angle = -acos(dot(camera, tree)) * 180 / 3.14159;
	Vector rotate = cross(camera, tree);
	
	rotate.normalize();
	

	for (map<double, int>::iterator i = sortedTrees.begin(); i != sortedTrees.end(); i++)
	{
		glPushMatrix(); {
			// TODO #2: Draw our sprite!
			glTranslatef(treeSpritePositions.at(i->second).at(0), treeSpritePositions.at(i->second).at(1), treeSpritePositions.at(i->second).at(2));
			glRotated(angle, rotate.at(0), rotate.at(1), rotate.at(2));
...
int main(int argc, char **argv) {
	...
	glutCreateWindow("Lab09: Billboarding");
	...
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	printf("[INFO]: ground texture read in and registerd\n");
