...
void myTimer(int value){
	...
	for (int i = 0; i < balls.size(); i++) {
		...
		if (balls.at(i)->location.getZ() < -groundSize) {
			balls.at(i)->moveBackward();
		...
		if (bounce) {
			Vector ballVec = balls.at(i)->direction;
			ballVec.normalize();
			Vector output = balls.at(i)->direction - 2 * dot(ballVec, normal) * normal;
			balls.at(i)->direction = output;
			bounce = false;
		}
	}
	...
	for (int i = 0; i < balls.size(); i++) {
		for (int j = i + 1; j < balls.size(); j++) {
			double x = balls.at(i)->location.getX() - balls.at(j)->location.getX();
			double y = balls.at(i)->location.getY() - balls.at(j)->location.getY();
			double z = balls.at(i)->location.getZ() - balls.at(j)->location.getZ();
			double distance = sqrt(x*x + y*y + z*z);
			if (distance < (balls.at(i)->radius + balls.at(j)->radius)) {
				//Collision detected
				...
				N2.normalize();
				//Do stuff for ball-i
				Vector ballVecI = balls.at(i)->direction;
				ballVecI.normalize();
				Vector outputI = balls.at(i)->direction - 2 * dot(ballVecI, N2) * N2;
				balls.at(i)->direction = outputI;
				//Do stuff for ball-j
				Vector ballVecJ = balls.at(j)->direction;
				ballVecJ.normalize();
				Vector outputJ = balls.at(j)->direction - 2 * dot(ballVecJ, N1) * N1;
				balls.at(j)->direction = outputJ;
				balls.at(i)->moveForward();
				balls.at(j)->moveForward();
			}
		}
	}
...
int main(int argc, char **argv) {
    ...
    glutCreateWindow("Lab10: Collision Detection");
