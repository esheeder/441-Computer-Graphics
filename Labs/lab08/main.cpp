...
enum LightType { POINT_LIGHT, DIRECTIONAL, SPOT };		// our types of lights
LightType lightType = POINT_LIGHT;				// current light type

GLuint texture[2];	//Array of textures
		...
		switch( currentObject ) {
            ...
            case OTHER:
				...
				glPushMatrix(); {
					...
					glTexCoord2f(2.0f, 0.0f); glVertex3f(3, -3, 0);
					glTexCoord2f(2.0f, 2.0f); glVertex3f(3, 3, 0);
					glTexCoord2f(0.0f, 2.0f); glVertex3f(-3, 3, 0);
					glEnd();
				} glPopMatrix();
                break;
		};
...
int main(int argc, char **argv) {
	...
	if (!readPPM("textures/brick.ppm", imgWidth, imgHeight, imgData)) {
		exit(1);
	}

    // TODO #2a: Register PPM Texture
	registerOpenGLTexture(imgData, imgWidth, imgHeight, texture[0]);*/
